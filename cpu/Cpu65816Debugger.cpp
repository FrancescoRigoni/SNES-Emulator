...
void Cpu65816Debugger::step() {
    ...
    const uint8_t instruction = mCpu.mMemoryMapper.readByte(mCpu.mProgramAddress);
    ...
    switch(opCode.getAddressingMode()) {
        ...
        case AddressingMode::Immediate:
            ...
            if (mCpu.accumulatorIs8BitWide()) {
                log.str("#").hex(mCpu.mMemoryMapper.readByte(mCpu.getAddressOfOpCodeData(opCode)), 2);
            } else {
                log.str("#").hex(mCpu.mMemoryMapper.readTwoBytes(mCpu.getAddressOfOpCodeData(opCode)), 4);
            }
            break;
        ...
        case AddressingMode::AbsoluteIndexedWithX:
            log.hex(mCpu.mMemoryMapper.readTwoBytes(onePlusOpCodeAddress), 4).str(", X").sp();
            log.str("                 [Absolute Indexed, X]");
            break;
        case AddressingMode::AbsoluteLongIndexedWithX:
        {
            Address opCodeDataAddress = mCpu.getAddressOfOpCodeData(opCode);
            Address effectiveAddress = mCpu.mMemoryMapper.readAddressAt(opCodeDataAddress);
        ...
        case AddressingMode::AbsoluteIndexedWithY:
            log.hex(mCpu.mMemoryMapper.readTwoBytes(mCpu.getAddressOfOpCodeData(opCode)), 4).str(", Y").sp();
            log.str("                    [Absolute Indexed, Y]");
            break;
        case AddressingMode::DirectPage:
            log.hex(mCpu.mMemoryMapper.readByte(onePlusOpCodeAddress), 2).sp();
            log.str("                      [Direct Page]");
            break;
        case AddressingMode::DirectPageIndexedWithX:
            log.hex(mCpu.mMemoryMapper.readByte(onePlusOpCodeAddress), 2).str(", X").sp();
            log.str("                    [Direct Page Indexed, X]");
            break;
        case AddressingMode::DirectPageIndexedWithY:
            log.hex(mCpu.mMemoryMapper.readByte(onePlusOpCodeAddress), 2).str(", Y").sp();
            log.str("                    [Direct Page Indexed, Y]");
            break;
        case AddressingMode::DirectPageIndirect:
            log.str("(").hex(mCpu.mMemoryMapper.readByte(onePlusOpCodeAddress), 2).str(")").sp();
            log.str("                    [Direct Page Indirect]");
            break;
        case AddressingMode::DirectPageIndirectLong:
            log.str("[").hex(mCpu.mMemoryMapper.readByte(mCpu.getAddressOfOpCodeData(opCode)), 2).str("]").sp();
            log.str("                    [Direct Page Indirect Long]");
            break;
        case AddressingMode::DirectPageIndexedIndirectWithX:
            log.str("(").hex(mCpu.mMemoryMapper.readByte(mCpu.getAddressOfOpCodeData(opCode)), 2).str(", X)").sp();
            log.str("                    [Direct Page Indexed Indirect, X]");
            break;
        case AddressingMode::DirectPageIndirectIndexedWithY:
            log.str("(").hex(mCpu.mMemoryMapper.readByte(mCpu.getAddressOfOpCodeData(opCode)), 2).str("), Y").sp();
        ...
        case AddressingMode::StackRelative:
            log.hex(mCpu.mMemoryMapper.readByte(mCpu.getAddressOfOpCodeData(opCode)), 2).str(", S").sp();
        ...
        case AddressingMode::StackRelativeIndirectIndexedWithY:
            log.str("(").hex(mCpu.mMemoryMapper.readByte(Address::sumOffsetToAddressWrapAround(mCpu.mProgramAddress, 1)), 2);
            log.str(", S), Y").sp();
            log.str("                    [Absolute Indexed, X]");
            break;
        case AddressingMode::ProgramCounterRelative:
            log.hex(mCpu.mMemoryMapper.readByte(mCpu.getAddressOfOpCodeData(opCode)), 2).sp();
